...
case "$ARCH" in
x86_64)
PRIMARY_URL="https://github.com/irOblivionSpark/bb-tunnel/raw/refs/heads/main/backhaul_premium.tar.gz"
FALLBACK_URL="https://github.com/irOblivionSpark/bb-tunnel/raw/refs/heads/main/backhaul_premium.tar.gz"
;;
arm64|aarch64)
...
SERVER_COUNTRY=$(curl -sS --max-time 1 "http://ipwhois.app/json/$SERVER_IP" | jq -r '.country')
SERVER_ISP=$(curl -sS --max-time 1 "http://ipwhois.app/json/$SERVER_IP" | jq -r '.isp')
check_config_backup() {
missing_services=()
